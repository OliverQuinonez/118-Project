...
b_int = np.array([f_to_b(omega0,20e-2),f_to_b(omega0,50e-2),f_to_b(omega0,100e-2)])
...
#PXe_int = np.array([10,25,50,75,100])
...
PAr_samples = 3
dk_int =np.linspace(PAr_to_dk(0,params_WA['PXe']),PAr_to_dk(500,params_WA['PXe']),PAr_samples)
dk_scan = xr.DataArray(dk_int,
                     dims = 'dk',
                     attrs = {'units': 'm^-1',
                              'long_name': "Phase Mismatch"})

B_scan = scan_builder(single_func, params_WA, [b_scan,dk_scan])
